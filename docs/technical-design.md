...
Templates default to the retained `fanruan_tech` layout in this checkout. Users may explicitly opt out with `no template` / `free design`, or select another compatible workspace by path.

**Why retain FanRuan by default.** This local repository is intentionally specialized for FanRuan BI, operating-report, and data-analysis decks. Free design remains available when the user opts out, and explicit workspace paths remain available for future compatible assets.

**Mechanical trigger, not semantic matching.** A bare non-default name, brand mention, or style phrase does not select another library directory. Silence uses `fanruan_tech`; an explicit opt-out uses free design; another workspace still requires an explicit path. Current Brand/Layout/Deck workspaces resolve `templates/design_spec.md`; compatible flat-directory packages resolve direct `design_spec.md` only when their SVGs already satisfy the current contract. Directory shape never authorizes structure migration. A package with legacy Master/Layout/placeholder semantics must be replaced by a newly created template workspace before Step 3.
