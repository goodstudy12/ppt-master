...
```yaml
# templates/brands/<brand_id>/templates/design_spec.md
---
kind: brand
...
---

# templates/layouts/<layout_id>/templates/design_spec.md
---
kind: layout
...
Field-level micro-adjustment goes through the existing Strategist confirmation stage path — the user says in chat "use this brand workspace but change primary to #FF0000", and Strategist adjusts fields e/g. Step 3 fusion does not add field-level syntax.

### Same-kind multiple paths = git-style conflict resolution

User supplies two future Brand workspaces (or any same-kind permutation):
...
```markdown
> **Fused from:**
> - deck: `templates/decks/<deck_id>/` (base)
> - brand: `templates/brands/<brand_id>/` (identity override)
> - layout: `templates/layouts/<layout_id>/` (structure override)
> - conflicts resolved: Color Scheme from the selected brand (user picked a)
```
