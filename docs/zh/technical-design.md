...
本地工作流默认使用保留的 `fanruan_tech` 布局。用户可以用“不要模板”/“自由设计”主动退出，也可以用显式路径选择其他兼容工作区。

**为什么默认保留帆软。** 本地仓库已经针对帆软 BI、经营报表和数据分析汇报做了专门裁剪。用户主动退出后仍可自由设计，未来兼容的工作区也可以通过显式路径接入。

**机械触发，不做语义匹配。** 非默认模板名、品牌提及或“麦肯锡风格”等风格短语，不会选择其他库目录。沉默时使用 `fanruan_tech`，主动退出时走自由设计，其他工作区必须提供显式路径。当前 Brand/Layout/Deck 工作区均解析 `templates/design_spec.md`；平铺目录只有在 SVG 已满足当前合同时，才兼容从根目录读取 `design_spec.md`。目录形态从不授权结构迁移；带旧 Master/Layout/placeholder 语义的包必须先替换为新建的模板工作区，才能进入 Step 3。
