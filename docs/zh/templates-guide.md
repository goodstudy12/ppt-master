...
| 已有可复用 Brand/Layout/Deck 工作区，想生成一份全新 deck | **Generate PPTX + 显式工作区路径** | `用 sources/report.pdf 做 deck，模板用 skills/ppt-master/templates/layouts/fanruan_tech/。` |
...
工作流默认使用保留的 `fanruan_tech` 布局。你可以写“不要模板”/“自由设计”主动退出，也可以提供显式工作区路径选择其他兼容工作区。
...
- **只写非默认模板名、不给路径**："用另一个模板" → 继续使用默认 `fanruan_tech`。AI 不会替你把名字解析成路径；要用其他模板，请直接给路径。
...
模板工作区：skills/ppt-master/templates/layouts/fanruan_tech/
```
...
Layout 工作区：skills/ppt-master/templates/layouts/fanruan_tech/
```
...
模板按三种 kind 分目录，并分别由发现索引维护。本地仓库有意保持 brands 与 decks 索引为空，layouts 只保留 `fanruan_tech`：
...
风格描述和模板名仍走**两套机制**：“极简风”是解释性语言，`fanruan_tech/` 是本地保留的真实模板目录，其他模板必须提供显式路径。
