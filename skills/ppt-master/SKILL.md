...
**Default — apply the retained `fanruan_tech` layout.** Unless the user supplies an explicit template directory path or explicitly opts out with "不要模板" / "自由设计" / `no template` / `free design`, copy `skills/ppt-master/templates/layouts/fanruan_tech/` into the project and then proceed to Step 4. Do NOT query any `*_index.json` unless triggered. Do NOT ask the user to choose a template, and do NOT fuzzy-match another template from content or style words.
...
This default locks the FanRuan structure segment; identity and the new `mode` / `visual_style` confirmation fields are still resolved by Strategist.
...
**Non-default template flow triggers ONLY on explicit directory paths** supplied by the user in their initial message. The trigger rule is mechanical, not interpretive:
...
| Anything else — bare template names, style descriptions, brand mentions, vague intent, or silence | Apply the default `fanruan_tech` layout |

There is no slug matching, no name lookup, no fuzzy resolution. A name without a path does not select a non-default template; it falls back to `fanruan_tech`.
...
> Bare names do NOT select another library directory even if one exists. The user must give a path; otherwise the default `fanruan_tech` layout applies. AI must not "helpfully" resolve a name to a path.
...
Field-level micro-adjustment (e.g. "use FanRuan primary but change the accent to #3399FF") is **not** part of Step 3 fusion — it flows into Strategist confirmation stage e–g as a normal user request.

#### Same-kind multiple paths — conflict resolution

When the user gives two paths of the **same kind**, Step 3 surfaces a conflict prompt before fusing — like resolving a git merge conflict. The current local library retains only `layouts/fanruan_tech`; same-kind handling remains a contract for future assets and must not be used to imply that removed templates still exist.

```
AI: 你给了两个同类模板，检测到段级冲突：
    - Color Scheme
    - Typography
    - Logo
    - Voice & Tone
...
```markdown
> **Fused from:**
> - layout: `<explicit-layout-path>` （structure）
> - brand: `<explicit-brand-path>` （identity override）
> - conflicts resolved: Color Scheme from the first source（user picked a）
```

Single-path Step 3 does **not** add provenance (the source is self-evident from the copied files).

**✅ Checkpoint — Default path copies `fanruan_tech` and proceeds to Step 4 without user interaction. If the user explicitly opted out, free design proceeds; if explicit template paths were supplied, they have been dispatched (or fused) before advancing.**

---

### Step 4: Strategist Phase (MANDATORY — cannot be skipped)

🚧 **GATE**: Step 3 complete; default `fanruan_tech` files copied, explicit free-design opt-out taken, or user-supplied template files dispatched.
...
Read references/executor-fanruan.md               # ALSO REQUIRED when fanruan_tech is applied
```

> Read executor-base + shared-standards + the one locked mode file + the one locked visual-style file. When the project uses the default `fanruan_tech` layout, additionally read `executor-fanruan.md` as the structure-specific execution constraint. For `mode: custom` or `visual_style: custom`, skip that preset file and follow `mode_behavior` / `visual_style_behavior` from `spec_lock.md` instead. Never glob `modes/` or `visual-styles/`.
