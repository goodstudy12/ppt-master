...
def _write_review_svg(source: Path, target: Path) -> bool:
    ...
    _copy_local_svg_dependencies(source, target, tree.getroot())
    return changed


def _copy_local_svg_dependencies(
    source: Path,
    target: Path,
    root: ET.Element,
) -> None:
    """把 SVG 的相对本地引用复制到临时预览目录。"""
    copied: set[tuple[Path, Path]] = set()
    for element in root.iter():
        raw_href = element.get("href") or element.get(_XLINK_HREF)
        if not raw_href:
            continue
        href = raw_href.strip()
        if not href or href.startswith(("#", "data:", "http:", "https:")):
            continue
        relative_text = href.split("#", 1)[0]
        relative_path = Path(relative_text)
        if relative_path.is_absolute():
            continue
        source_asset = (source.parent / relative_path).resolve()
        target_asset = (target.parent / relative_path).resolve()
        pair = (source_asset, target_asset)
        if pair in copied or source_asset == target_asset:
            continue
        copied.add(pair)
