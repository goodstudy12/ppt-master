...
**Default — apply the retained `fanruan_tech` layout**: Unless the user explicitly opts out with "不要模板" / "自由设计" / `no template` / `free design`, install the retained structured FanRuan layout into the project and continue to Step 4. Do not query any `*_index.json`, ask about templates, suggest another local template, or fuzzy-match a name from content, brand mentions, or style language.
...
The local library intentionally keeps only this layout. Its flat directory shape remains compatible with the v4 template reader, while its SVGs declare the required structured Master/Layout metadata. Do not restore removed brand/deck/layout assets to satisfy catalog assumptions.
...
Bare names, style descriptions, brand mentions, and vague template intent do not select a non-default template. Silence selects `fanruan_tech`; explicit opt-out selects free design. There is no slug lookup or fuzzy path resolution.
...
| Confirmed style explicitly names FanRuan / FineReport / FineBI, or the deck is primarily a KPI dashboard, operating report, BI analysis, or management cockpit | `executor-fanruan.md` |
